          ...
          {movies.map((item) => (
            <div key={item.id} className="inline-block cursor-pointer relative p-2 z-10" onClick={() => navigate(`/${item.id}`)}>
              <img
                ...
                alt=""
              />
              <div  className="absolute top-0 left-0 w-full h-full hover:bg-black/60 opacity-0 hover:opacity-100 text-[#FFFDE3] z-20">
                <div onClick={(e) => {
                  e.stopPropagation();
                  deleteMovie(item.id)
                  }} 
                  className="absolute text-[#FFFDE3] text-xl top-4 right-4 z-20">
                  <HiTrash />
                </div>
              </div>
            </div>
